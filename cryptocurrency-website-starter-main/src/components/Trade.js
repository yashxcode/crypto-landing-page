...
const Trade = () => {
  ...
  return (
    <section className="section lg:pt-[320px] bg-gradient-to-b 
      ...
      <div className="container mx-auto">
        ...
        data-aos="fade-up" data-aos-offset="450">
          {currency.map((item, index) => {
            const { image, name, abbr, description } = item   // destructuring item
            return (
              <div key={index} onClick={() => setItemName(name)} 
              className={`${name === itemName ? "bg-violet text-white" : "bg-white"} w-full rounded-2xl py-12 px-6 
              shadow-primary cursor-pointer transition duration-300`}>
                <div className="flex flex-col justify-center items-center">
                  <img src={image} className="mb-12" />
                  ...
                  <button className={`${name === itemName ? 
